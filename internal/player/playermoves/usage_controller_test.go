...
func TestUsageControllerInitialize(t *testing.T) {
	...
	require.Len(t, state.Player.Moves.Moves[0].ActiveCards, 2)
	assert.Equal(t, domain.Blue, state.Player.Moves.Moves[0].ActiveCards[0].Move)
	assert.Equal(t, domain.Red, state.Player.Moves.Moves[1].Move)
	assert.Equal(t, 4, state.Player.Moves.Moves[1].MaxUses)
	assert.Equal(t, 4, state.Player.Moves.Moves[1].RemainingUses)
	assert.Equal(t, domain.Yellow, state.Player.Moves.Moves[2].Move)
	assert.Equal(t, 1, state.Player.Moves.Moves[2].MaxUses)
	assert.Equal(t, 1, state.Player.Moves.Moves[2].RemainingUses)
}
...
func TestUsageControllerPrepareRound(t *testing.T) {
	...
	t.Run("restores a deck when its recharge round has arrived and shuffles it", func(t *testing.T) {
		...
		state := match.State{
			...
			Player: match.PlayerState{Moves: match.PlayerMoveResources{Moves: []match.PlayerMoveState{
		...
		assert.Equal(t, 2, state.Player.Moves.Moves[0].RemainingUses)
		assert.Equal(t, 2, state.Player.Moves.Moves[0].MaxUses)
		assert.Equal(t, 0, state.Player.Moves.Moves[0].RestoresOnRound)
		require.Len(t, state.Player.Moves.Moves[0].ActiveCards, 2)
		assert.Equal(t, 1, state.Player.Moves.Moves[0].ActiveCards[0].Effects[0].SurfaceDepthBonus)
		assert.Empty(t, state.Player.Moves.Moves[0].DiscardedCards)
	})

	t.Run("keeps a deck blocked when its recharge round has not arrived yet", func(t *testing.T) {
		state := match.State{
			Round: match.RoundState{Number: 1},
			Player: match.PlayerState{Moves: match.PlayerMoveResources{Moves: []match.PlayerMoveState{
		...
		assert.Empty(t, state.Player.Moves.Moves[0].ActiveCards)
		require.Len(t, state.Player.Moves.Moves[0].DiscardedCards, 2)
	})
}
...
func TestUsageControllerValidateMove(t *testing.T) {
	...
	t.Run("returns nil when the selected color still has cards remaining", func(t *testing.T) {
		state := match.State{Player: match.PlayerState{Moves: match.PlayerMoveResources{Moves: []match.PlayerMoveState{
...
func TestUsageControllerConsumeMove(t *testing.T) {
	...
	t.Run("consumes the visible card and reveals the next one", func(t *testing.T) {
		...
		assert.Equal(t, 2, state.Player.Moves.Moves[0].MaxUses)
		require.Len(t, state.Player.Moves.Moves[0].ActiveCards, 1)
		assert.Equal(t, secondCard, state.Player.Moves.Moves[0].ActiveCards[0])
		require.Len(t, state.Player.Moves.Moves[0].DiscardedCards, 1)
		assert.Equal(t, firstCard, state.Player.Moves.Moves[0].DiscardedCards[0])
		assert.Equal(t, 0, state.Player.Moves.Moves[0].RestoresOnRound)
	})
	...
	t.Run("schedules recovery when the last card in a color deck is consumed", func(t *testing.T) {
		...
		state := match.State{
			Round: match.RoundState{Number: 3},
			Player: match.PlayerState{Moves: match.PlayerMoveResources{Moves: []match.PlayerMoveState{
		...
		assert.Equal(t, 0, state.Player.Moves.Moves[0].RemainingUses)
		assert.Equal(t, 1, state.Player.Moves.Moves[0].MaxUses)
		assert.Equal(t, 5, state.Player.Moves.Moves[0].RestoresOnRound)
		assert.Empty(t, state.Player.Moves.Moves[0].ActiveCards)
		require.Len(t, state.Player.Moves.Moves[0].DiscardedCards, 1)
	})
}
